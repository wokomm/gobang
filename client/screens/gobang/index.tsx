...
export default function GobangPage() {
  ...
  const checkWin = useCallback(
    (board: Board, row: number, col: number, player: 'black' | 'white'): boolean => {
      const directions = [
        [0, 1],
        [1, 0],
        [1, 1],
  ...
  const handlePress = useCallback(
    ...
    [board, currentPlayer, winner, checkWin, resetGame]
  );
    ...
    for (let row = 0; row < BOARD_SIZE; row++) {
      for (let col = 0; col < BOARD_SIZE; col++) {
        ...
        cells.push(
          ...
          >
            <View
              style={[styles.gridLine, styles.horizontalLine, { top: CELL_SIZE / 2 }]}
            />
  ...
  return (
    <Screen statusBarStyle="dark">
      <View className="flex-1 items-center justify-start pt-4">
        ...
        <View style={[styles.board, { padding: CELL_SIZE / 2 }]}>
